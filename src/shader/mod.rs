...
pub fn get_phong(color: Rgba<f64>) -> Box<Shader> {
    ...
    let comb1 = AdditiveShader { shader1: Box::new(diffuse_shader), shader2: Box::new(specular_shader)};
    let comb2 = AdditiveShader { shader1: Box::new(ambient_shader), shader2: Box::new(comb1)};
