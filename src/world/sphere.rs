...
impl Interceptable for Sphere {
    fn intercept(&self, ray: &Ray) -> Option<(f64, Intersection)> {
        ...
        let a = 1.0; // scalar
        let b = 2.0 * h.component_mul(&ray.dir.unwrap()); // vector, needs to be summed/normed before utilisation
        let c = h.norm_squared() - self.radius.powi(2); // scalar

        let delta = b.norm_squared() - 4.0 * a * c;

        let epsylon = 0.0001; //TODO: adjust precision (do we need more/less?)

        let b_summed : f64 = b.iter().sum();
        ...
        return if delta > epsylon {
            ...
            let lambda_1 = - b_summed + delta.sqrt();
            let lambda_1 = lambda_1 / (2.0*a);

            let lambda_2 = - b_summed - delta.sqrt();
            let lambda_2 = lambda_2 / (2.0*a);

            let min_pos_lambda =  if lambda_1 > epsylon && lambda_2 > epsylon { lambda_1.min(lambda_2) } else { lambda_1.max(lambda_2) };
            ...
            let angle = (pos_to_center.dot(&-ray.dir) / (pos_to_center.norm() * ray.dir.norm())).acos() - (std::f64::consts::PI / 4.0);
